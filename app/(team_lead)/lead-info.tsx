...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, ActivityIndicator, Linking, Modal, TextInput, Alert } from 'react-native';
import { useLocalSearchParams, router } from 'expo-router';
import { useData } from '@/contexts/DataContext';
import { useTheme } from '@/contexts/ThemeContext';
import { Info, ArrowLeft, Phone, Clock, MessageCircle, FileText } from 'lucide-react-native';

const TeamLeadLeadInfo = ({ routerBase = '/(team_lead)' }) => {
  const { id, leadId } = useLocalSearchParams();
  const { leads, isLoading, getCallLogs, getCallLaterLogs } = useData();
  const { theme } = useTheme();
  const styles = getStyles(theme);

  const lead_id = id || leadId;

  // Utility to normalize lead fields to snake_case and handle nulls
  ...
  const lead = useMemo(() => {
    if (!lead_id || leads.length === 0) {
      return null;
    }
    const found = leads.find(l => l.id === lead_id);
  ...
  const sortedLogs = useMemo(() => lead ? [...getCallLogs(lead.id)].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()) : [], [lead, getCallLogs]);
  ...
  const sortedCallLaterLogs = useMemo(() => [...callLaterLogs].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()), [callLaterLogs]);
  ...
  if (isLoading || !lead) {
    return (
      <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}> 
        ...
        {!isLoading && (
          <Text style={{ color: theme.error, marginTop: 16 }}>Lead not found.</Text>
        )}
      </View>
    );
  }
  ...
  // Action handlers
  const handleCall = () => Linking.openURL(`tel:${lead.phone_number}`);
  const handleWhatsApp = () => Linking.openURL(`https://wa.me/${lead.phone_number.replace(/\D/g, '')}`);
...
export default TeamLeadLeadInfo; 
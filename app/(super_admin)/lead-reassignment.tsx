...
export default function LeadReassignmentScreen() {
  ...
  const allSelected = filteredOperatorLeads.length > 0 && filteredOperatorLeads.every(lead => selectedLeads.includes(lead.id));
  ...
  const toggleSelectAll = () => {
    if (allSelected) {
      setSelectedLeads(selectedLeads.filter(id => !filteredOperatorLeads.some(lead => lead.id === id)));
    } else {
      setSelectedLeads([...new Set([...selectedLeads, ...filteredOperatorLeads.map(lead => lead.id)])]);
    }
  };
    ...
    return (
      <View style={styles.leadCard}>
        <View style={styles.leadHeader}>
          <TouchableOpacity onPress={() => toggleLeadSelection(lead.id)} style={{ marginRight: 8 }}>
            <View style={{ width: 20, height: 20, borderRadius: 4, borderWidth: 2, borderColor: '#1E40AF', backgroundColor: selectedLeads.includes(lead.id) ? '#1E40AF' : '#FFF', justifyContent: 'center', alignItems: 'center' }}>
              {selectedLeads.includes(lead.id) && <View style={{ width: 12, height: 12, backgroundColor: '#FFF', borderRadius: 2 }} />}
            </View>
          </TouchableOpacity>
          <TouchableOpacity style={{ flex: 1 }} onPress={() => router.push({ pathname: '/(super_admin)/lead-info', params: { id: lead.id } })}>
      ...
      <View style={styles.content}>
        ...
        {selectedOperator && (
          <View style={styles.leadsSection}>
            ...
            <TextInput
              style={{ borderWidth: 1, borderColor: '#E2E8F0', borderRadius: 6, marginBottom: 8, padding: 8 }}
              placeholder="Search leads by name, phone, or address"
              value={searchQuery}
              onChangeText={setSearchQuery}
            />
            <TouchableOpacity onPress={toggleSelectAll} style={{ marginBottom: 8, flexDirection: 'row', alignItems: 'center' }}>
              <View style={{ width: 20, height: 20, borderRadius: 4, borderWidth: 2, borderColor: '#1E40AF', backgroundColor: allSelected ? '#1E40AF' : '#FFF', justifyContent: 'center', alignItems: 'center', marginRight: 8 }}>
                {allSelected && <View style={{ width: 12, height: 12, backgroundColor: '#FFF', borderRadius: 2 }} />}
              </View>
              <Text style={{ color: '#1E40AF', fontWeight: 'bold' }}>Select All</Text>
            </TouchableOpacity>
            ...
            {totalPages > 1 && (
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 8 }}>
                <TouchableOpacity
                  style={{ marginHorizontal: 8, padding: 8, backgroundColor: '#1E40AF', borderRadius: 6, opacity: currentPage === 1 ? 0.5 : 1 }}
                  onPress={() => setCurrentPage(p => Math.max(1, p - 1))}
                  disabled={currentPage === 1}
                >
                  <Text style={{ color: '#FFF' }}>Previous</Text>
                </TouchableOpacity>
                <Text style={{ color: '#1E40AF', marginHorizontal: 8 }}>Page {currentPage} of {totalPages}</Text>
                <TouchableOpacity
                  style={{ marginHorizontal: 8, padding: 8, backgroundColor: '#1E40AF', borderRadius: 6, opacity: currentPage === totalPages ? 0.5 : 1 }}
                ...
                {/* Page Size Selector */}
                <Text style={{ marginLeft: 16, color: '#1E40AF' }}>Page Size:</Text>
                <TextInput
                  style={{ borderWidth: 1, borderColor: '#E2E8F0', borderRadius: 6, width: 48, marginLeft: 8, padding: 4, textAlign: 'center', color: '#1E40AF' }}
            ...
            {selectedLeads.length > 0 && (
              <TouchableOpacity style={[styles.reassignButton, { marginTop: 12 }]} onPress={() => setShowReassignmentModal(true)}>
                <ArrowRight size={16} color="#FFFFFF" />
                <Text style={styles.reassignButtonText}>Reassign Selected ({selectedLeads.length})</Text>
              </TouchableOpacity>
            )}
          </View>
        )}
